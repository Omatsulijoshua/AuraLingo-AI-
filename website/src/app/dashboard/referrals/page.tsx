...
export default function ReferralDashboard() {
  ...
  // Active Filter Bounds (applied on clicking 'Filter')
  ...
  const filteredReferrals = stats?.referralsList?.filter((refUser: any) => {
    if (!refUser.createdAt) return false;
  ...
  const filteredCount = filteredReferrals.length;
  
  const filteredEarnings = filteredReferrals.reduce((sum: number, ref: any) => {
    return sum + (ref.isPaidUser ? (ref.rewardEarned || stats?.rewardPerUser || 1000) : 0);
  }, 0);

  const filteredPending = filteredReferrals.reduce((sum: number, ref: any) => {
    return sum + (!ref.isPaidUser ? (ref.rewardEarned || stats?.rewardPerUser || 1000) : 0);
  }, 0);
  ...
  return (
    <div className="min-h-screen bg-navy text-white flex flex-col">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-8 space-y-8">
        {/* Notice Banner */}
        <div className="bg-amber-500/10 border border-amber-505/30 text-amber-500 p-4 rounded-2xl flex items-center gap-3 text-xs md:text-sm font-semibold">
          <span className="text-lg">📢</span>
          <p><strong>Notice:</strong> Payment of referral earnings is processed on the <strong>21st of every month</strong>.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-primary/25 border border-primary-light/30 rounded-2xl p-6 shadow-xl space-y-4 md:col-span-2">
            ...
            <div className="bg-navy/40 border border-primary-light/20 p-4 rounded-xl space-y-3 sm:space-y-0 sm:flex sm:items-end sm:gap-4">
              <div className="flex-1">
                <label className="block text-slate-500 text-[9px] uppercase font-bold tracking-widest mb-1.5">Start Date</label>
                <input
                  type="date"
                  value={startDateInput}
                  onChange={(e) => setStartDateInput(e.target.value)}
                  className="w-full bg-navy border border-primary-light/30 focus:border-gold rounded-lg px-3 py-2 text-xs text-white focus:outline-none cursor-pointer"
                />
              </div>
              <div className="flex-1">
                <label className="block text-slate-505 text-[9px] uppercase font-bold tracking-widest mb-1.5">End Date</label>
                <input
                  type="date"
                  value={endDateInput}
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  onClick={() => {
                    setFilterStart(startDateInput);
                    setFilterEnd(endDateInput);
                  }}
                  className="bg-gold hover:bg-gold-dark text-primary font-black px-4 py-2 rounded-lg text-xs transition-colors cursor-pointer flex items-center gap-1"
                >
                  🔍 Filter
                </button>
                <button
                  ...
                  className="bg-navy/60 border border-primary-light/30 hover:border-white text-white font-bold px-4 py-2 rounded-lg text-xs transition-colors cursor-pointer"
                >
                  Clear
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div className="bg-navy/40 p-4 rounded-xl border border-primary-light/20">
                <p className="text-slate-500 text-[10px] uppercase font-bold tracking-wider">
                  {filterStart || filterEnd ? 'Earnings in Range' : 'Total Balance'}
                </p>
                <p className="text-white text-xl font-black mt-1">
                  ₦{(filterStart || filterEnd ? (filteredEarnings + filteredPending) : (stats?.referralBalance ?? 0)).toLocaleString()}
                </p>
              </div>
              <div className="bg-navy/40 p-4 rounded-xl border border-emerald-500/10">
                <p className="text-emerald text-[10px] uppercase font-bold tracking-wider">
                  {filterStart || filterEnd ? 'Paid (Withdrawable)' : 'Withdrawable'}
                </p>
                <p className="text-gold text-xl font-black mt-1">
                  ₦{(filterStart || filterEnd ? filteredEarnings : (stats?.withdrawableBalance ?? 0)).toLocaleString()}
                </p>
              </div>
              <div className="bg-navy/40 p-4 rounded-xl border border-amber-500/10">
                ...
                <p className="text-amber-500 text-xl font-black mt-1">
                  ₦{(filterStart || filterEnd ? filteredPending : (stats?.lockedBalance ?? 0)).toLocaleString()}
                </p>
              </div>
              <div className="bg-navy/40 p-4 rounded-xl border border-green-500/10">
                <p className="text-green-400 text-[10px] uppercase font-bold tracking-wider">
                  {filterStart || filterEnd ? 'Referred Signups' : 'This Month'}
                </p>
                <p className="text-green-400 text-xl font-black mt-1">
                  {filterStart || filterEnd 
                    ? `${filteredCount} users`
                    : `₦${(stats?.madeThisMonth ?? 0).toLocaleString()}`
                  }
                </p>
              </div>
            </div>
          ...
          <div className="bg-primary/25 border border-primary-light/30 rounded-2xl p-6 shadow-xl md:col-span-2 space-y-6">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                {(filterStart || filterEnd) && (
                  <span className="text-[10px] bg-gold/15 text-gold border border-gold/25 px-2 py-0.5 rounded font-bold">
                    Filtered Range
                  </span>
                )}
              </div>
              ...
              {filteredReferrals.length === 0 ? (
                <p className="text-slate-500 text-xs py-6 text-center">
                  {filterStart || filterEnd 
                    ? 'No referred signups found for the selected date range.' 
                    : 'No referred signups yet. Share your link to start earning!'
                  }
                </p>
              ) : (
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {filteredReferrals.map((refUser: any) => (
