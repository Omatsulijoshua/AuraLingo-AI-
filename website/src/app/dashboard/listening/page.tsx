...
export default function ListeningPractice() {
  ...
  const fetchQuestions = async () => {
    try {
      // 1. Fetch modules to resolve Listening ID
      const modules = await api.request<any[]>('/content/modules');
      const listeningMod = modules.find((m) => m.name.toUpperCase() === 'LISTENING');
      if (!listeningMod) {
        setQuestions([]);
        return;
      }
      ...
      const data = await api.request<any[]>(`/content/questions?moduleId=${listeningMod.id}`);
      setQuestions(data || []);
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="bg-primary/25 border border-primary-light/30 rounded-2xl p-6 shadow-xl space-y-6 self-start">
          ...
          <div>
            ...
            {questions.length === 0 ? (
              <p className="text-xs text-slate-500 italic">No listening questions generated yet. Go to Admin Dashboard to spin questions.</p>
            ) : (
              <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-1">
                {questions.map((q, idx) => (
                  ...
                  >
                    ...
                    {q.instruction || 'Listen and answer the question'}
                  </button>
                ))}
              </div>
            )}
          ...
          {selectedQuestion ? (
            <div className="bg-primary/25 border border-primary-light/30 rounded-2xl p-6 shadow-xl space-y-4">
              <div className="flex justify-between items-center border-b border-primary-light/20 pb-3">
                <h2 className="text-white font-bold text-lg">Listening Workspace</h2>
                {timerActive && (
              ...
              {/* Audio player if listeningAudio exists */}
              {selectedQuestion.listeningAudio && (
                <div className="bg-navy/40 border border-primary-light/10 p-4 rounded-xl space-y-2">
                  ...
                  <audio 
                    src={selectedQuestion.listeningAudio.audioUrl} 
                    controls 
            ...
            <div className="bg-primary/25 border border-primary-light/30 rounded-2xl p-12 text-center text-slate-500 text-xs shadow-xl">
              Select a listening question from the sidebar to begin.
            </div>
          )}
